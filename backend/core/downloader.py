    ...
    if not request.use_proxy:
        if not download_manager.can_start_download(str(request.url)):
            ...
            if len(download_manager.all_downloads) >= download_manager.MAX_TOTAL_DOWNLOADS:
                ...
                return {"id": db_req.id, "status": StatusEnum.waiting.value, "message": f"전체 다운로드 제한 도달 ({download_manager.MAX_TOTAL_DOWNLOADS}개). 대기 중..."}
            else:
                print(f"[LOG] 1fichier 로컬 다운로드 제한 도달 ({download_manager.MAX_LOCAL_DOWNLOADS}개). 대기 상태로 설정: {db_req.id}")
                return {"id": db_req.id, "status": StatusEnum.waiting.value, "message": f"1fichier 로컬 다운로드 제한 도달 ({download_manager.MAX_LOCAL_DOWNLOADS}개). 대기 중..."}
